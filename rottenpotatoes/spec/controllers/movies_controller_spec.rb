...
describe MoviesController do
  ...
  it 'should render two movies' do	    it 'should render two movies' do
    get :index	      get :index
    response.should render_template :index	      response.should render_template :index
  end	    end


  it 'should update render edit view' do	    it 'should update render edit view' do
    Movie.should_receive(:find).with('1')	      Movie.should_receive(:find).with('1')
    get :edit,	      get :edit,
        {id: '1'}	          {id: '1'}
  end	    end


  it 'should update data correctly' do	    it 'should update data correctly' do
    Movie.stub(:find).and_return(@movie1)	      Movie.stub(:find).and_return(@movie1)
    put :update,	      put :update,
        :id => @movie1[:id],	          :id => @movie1[:id],
        :movie => {title: "new_title", rating: "G", description: "test", release_date: "test", director: "test"}	          :movie => {title: "new_title", rating: "G", description: "test", release_date: "test", director: "test"}
  ...
  it 'should delete data correctly' do	    it 'should redirect to homepage on invalid no director request' do
    Movie.stub(:find).and_return(@movie1)	      Movie.should_receive(:similar_movies).with(@movie3[:id], {'director' => @movie3[:director]})
    delete :destroy,	      Movie.stub(:similar_movies).with(@movie3[:id], {'director' => @movie3[:director]}).and_return(nil)
        :id => @movie1[:id]	      get :similar, :id => @movie3[:id], :criteria => 'director'
    expect(flash[:notice]).to be_present	      expect(flash[:notice]).to be_present
      response.should redirect_to movies_path
    end
  end	  end
